...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $is_admin = 0; // Force admin to be approved manually
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // Insert user
        // Generate simple business ID (random 6-digit number)
        $business_id = mt_rand(100000, 999999);

        // Insert user with business_id
        $insert_query = "INSERT INTO users (company_name, business_id, contact_name, email, password_hash, phone, is_admin)
                VALUES ('$company', '$business_id', '$contact', '$email', '$password_hash', '$phone_number', '$is_admin')";


        if (mysqli_query($conn, $insert_query)) {
            $message = "Registration successful! Your Business ID is <strong>$business_id</strong>. Please keep this safe. You can now <a href='login.php'>login</a>.";
        } else {
...
<main class="form-container">
    ...
    <form method="post" action="register.php" class="form">
        ...
        <label><input type="checkbox" name="is_admin" value="0"> Request Administrator Access</label>
